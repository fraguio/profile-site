...
| 3. Variante B | Completada | Se construyó la dirección sistema en evolución con un hero oscuro y una metáfora abstracta de capas que se completa al abrir evidencias reales mediante `details` nativos. Incluye el contexto Abanca, las tres intervenciones Evolutio con sus solapes explícitos, origen, proyectos y aprendizaje; desktop usa escenarios persistentes y mobile los reduce a acompañamiento vertical. La crítica detectó que la evidencia debía usar lenguaje recruiter-first; la única pasada `clarify` nombró caso Abanca, caso Evolutio y cada evidencia por cargo y organización, y corrigió el overflow de CTAs mobile. | `pnpm check:prototype:interactive-experience` y `pnpm build:prototype:interactive-experience` correctos. Smoke de A, B y C en `1280x800` y `390x844` sin overflow; flechas verificadas; B respeta `prefers-reduced-motion`; fallback sin JavaScript visible con 4 CTAs y 19 artículos. El detector de código no registró hallazgos; sus avisos desktop y mobile, outputs y manifiesto están en `src/prototypes/interactive-experience/evidence/session-3-variant-b.*`. | Sin bloqueos. La sesión 4 puede construir exclusivamente la variante C desde este mismo `worktree` y rama. |
| 4. Variante C | Completada | Se construyó la dirección dossier de modernización con portada orientada al posicionamiento, índice persistente de ámbitos y lector de evidencias fechado. Abanca recibe el peso principal; las tres intervenciones de Evolutio se agrupan de forma explícita como registros solapados; origen, proyectos y aprendizaje conservan detalle mediante `details` nativos. La crítica está en `.impeccable/critique/active-experience-variants-variantc-astro-2026-09-22.md`; la única pasada `clarify` convirtió contacto en acción principal, hizo explícita la relación entre continuidad corporativa y modernización y reforzó la lectura y el foco de las evidencias. | `pnpm check:prototype:interactive-experience` y `pnpm build:prototype:interactive-experience` correctos. Smoke de A, B y C en `1280x800` y `390x844` sin overflow; `ArrowRight` cambia C a A; C respeta `prefers-reduced-motion`; fallback sin JavaScript visible con 19 artículos y al menos 3 enlaces. El detector de código no registra hallazgos; desktop conserva ocho avisos de medida de línea y mobile cuatro avisos de padding en el índice horizontal, registrados junto al manifiesto en `src/prototypes/interactive-experience/evidence/session-4-variant-c.*`. | Sin bloqueos. Los avisos del detector quedan como evidencia para comparación, sin abrir otra pasada de refinamiento. La sesión 5 puede evaluar A, B y C desde este mismo `worktree` y rama. |
